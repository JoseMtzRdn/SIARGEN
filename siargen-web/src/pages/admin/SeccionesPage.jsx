...
import { Plus, Search, Edit2, Trash2, Bookmark, Loader2, ChevronLeft, ChevronRight, Filter, X, RefreshCcw, Layers } from 'lucide-react';
import { useSelector, useDispatch } from 'react-redux';
import { fetchSecciones, deleteSeccion, fetchFondos } from '../../store/archivisticaSlice';
import Swal from 'sweetalert2';
import SeccionForm from '../../components/admin/SeccionForm';

const SeccionesPage = () => {
  const dispatch = useDispatch();
  const { items: secciones, loading, lastFetch } = useSelector((state) => state.archivistica.secciones);
  ...
  const fetchData = useCallback(async (search = searchTerm, activeFilters = filters) => {
    dispatch(fetchSecciones({ 
      search,
      fondo_id: activeFilters.fondo_id
    }));
  ...
  useEffect(() => {
    ...
    if (fondos.length === 0) {
      dispatch(fetchFondos());
    }
  }, [fetchData, lastFetch, dispatch, fondos.length]);
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      ...
      text: "Esta acción eliminará la sección y todas sus series documentales asociadas.",
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        ...
        const errorMessage = typeof error === 'string' ? error : (error?.message || 'No se pudo eliminar la sección');
        Swal.fire({
          icon: 'error',
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-4xl font-black text-gray-900 tracking-tight flex items-center gap-4">
            <Bookmark className="text-[#7A152E]" size={40} />
            Gestión de Secciones
          </h2>
          <p className="text-gray-500 font-bold mt-2 text-lg text-[#BC955B] uppercase tracking-wider">Nivel 2 • Sección Funcional</p>
        </div>
        <button
          onClick={() => { setSelectedSeccion(null); setIsModalOpen(true); }}
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-gray-100 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
           ...
           <select 
             ...
             value={filters.fondo_id}
             onChange={(e) => setFilters({...filters, fondo_id: e.target.value})}
           >
          ...
          <>
              ...
              <table className="w-full text-left border-collapse min-w-[900px]">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100">
                    ...
                    <th className="px-8 py-8 text-[11px] font-black text-gray-400 uppercase tracking-[0.25em]">Sección / Fondo Padre</th>
                    <th className="px-8 py-8 text-[11px] font-black text-gray-400 uppercase tracking-[0.25em] text-center w-32">Series</th>
                    ...
                    <tr key={s.id} className="hover:bg-gray-50/30 transition-colors group">
                      ...
                      <td className="px-10 py-8">
                        <div className="flex flex-col">
                           ...
                           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1.5 flex items-center gap-2">
                             <Layers size={10} className="text-[#BC955B]" /> {s.fondo?.nombre}
                           </p>
                        </div>
                      </td>
                      ...
                      <td className="px-10 py-8 text-center">
                        <div className="flex justify-center gap-4 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
                          ...
                          <button onClick={() => handleDelete(s.id)} className="p-4 bg-white text-red-400 hover:bg-red-500 hover:text-white rounded-2xl shadow-lg border border-gray-100 transition-all active:scale-90">
                            <Trash2 size={20} />
                          </button>
                        </div>
                      </td>
            ...
            <div className="px-10 py-6 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                Total de registros: <span className="text-[#7A152E] font-black">{secciones.length}</span>
              </p>
            </div>
          </>
        )}
